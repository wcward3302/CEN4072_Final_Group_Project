// Test class = Sign up
// Goal = go to sign up page, test different passwords and username
// Author = William Ward
...
public class Signup {
    ...
    //Test 2 - check if title of current page is correct
    @Test (priority = 2)
    ...
    //Test 3 - begin filling field with information
    @Test (priority = 3)
    ...
    //Test 4 - Begin testing passwords, 2 bad, 1 good
    @Test (priority = 4)
    ...
    void test_email_use() throws InterruptedException {
        ...
        // get error message, if empty, then test fail
        // Issue with site - when using automation, it will not confirm if address is currently in use - for bot protection
        Assert.assertEquals(error_text, "", "no error is present");
        Thread.sleep(2000);
    }

    //Test 6 - close and quit driver
    @Test (priority = 6)
